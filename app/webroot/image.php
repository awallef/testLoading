...
$image			= $_GET['image']; //preg_replace('/^(s?f|ht)tps?:\/\/[^\/]+/i', '', (string) $_GET['image']);
...
preg_match('/&text=/', $image, $match);

$ext = substr($image, ( strlen($image) - 4 ) , 4);
if( $ext != '.jpg' && $ext != '.JPG' && $ext != '.png' && empty($match) )
{
    $ext = substr($image, ( strlen($image) - 5 ) , 5);
    if( $ext != '.JPEG' && $ext != '.jpeg' )
    {
        header('HTTP/1.1 400 Bad Request');
        echo 'this kind of file is not supported ' + $ext;
        exit();
    }  
}
...
if ($start == 'http')
{       
    $tmp = CACHE_DIR . md5( $image );
    
    if( !file_exists($tmp) ){
        $imageString = file_get_contents($image);
        file_put_contents( $tmp ,$imageString );
    }
    $image = $tmp;
    $docRoot = '/';
...
if (!file_exists($docRoot . $image) &&  !file_exists($image) )
{
...
if (isset($_GET['valign']))
	$resizedImageSource	.= 'x' . (string) $_GET['valign'];
